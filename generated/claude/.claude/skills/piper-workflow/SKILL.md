...
review gates, and `automation-policy` before an `external` or `exceptional`
action.
...
| Execute one clear active-work wave, explicit slice, or optional queued task | Ralph | `/ralph` and Ralph sections in `STATION.md`; project source edits in the lane's checkout are routine |
| Review an implemented wave, group, or slice | Review | `review` |
| Push, pull request, dependency, networked command with effects, CI, or other `external` or `exceptional` action | Finish or the boundary ask | `automation-policy` |
...
worktree) is writable in the active session. If the lane's checkout (`repo_path` or its recorded worktree) is outside the hub, ensure Claude Code has workspace access through `/add-dir <checkout-path>` or `claude --add-dir <checkout-path>` before editing.
...
- Do not push, merge to a remote, open a pull request, install dependencies,
  ...
  Commits, local merges, and worktree changes are routine once reached.
  Delete, force-push, rewrite pushed history, deploy to
