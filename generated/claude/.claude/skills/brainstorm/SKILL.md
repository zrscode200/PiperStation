...
7. If the lane's checkout (`repo_path` or its recorded worktree) is outside the hub, ensure Claude Code has workspace access through `/add-dir <checkout-path>` or `claude --add-dir <checkout-path>` before editing.
