...
# Internal implementation. Public entry points run register.py, which holds
# piper-record's process-owned lock across every registration read and write.
[ "${PIPER_REGISTRATION_LAUNCHER:-}" = "1" ] || {
  echo "Error: use bin/add-project or bootstrap/add-project.sh, not registration-body.sh directly" >&2
  exit 1
}
...
display_re = re.compile(r"^\s*-\s+Display name:\s*(.+?)\s*$")
...
validate_description
validate_existing_project
if [ "$WRITE_REPO_MARKERS" = true ]; then validate_existing_repo_marker; fi
validate_repo_path_uniqueness
