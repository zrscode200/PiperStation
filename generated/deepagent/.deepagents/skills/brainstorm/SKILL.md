...
Read `.deepagents/AGENTS.md` and `STATION.md` first. Use the other root docs as
...
7. The agent works in the hub directory it was launched from, and the hub must be a git repository root for hub surfaces to load. Use absolute paths when reading or editing files in registered project repos; relative paths do not resolve against the working directory.
