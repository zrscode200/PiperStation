...
worktree) is writable in the active session. If the lane's checkout (`repo_path` or its recorded worktree) is outside the current working directory, open OpenCode from that directory or adjust workspace access before editing.
