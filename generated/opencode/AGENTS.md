...
`exceptional` are the only Piper-level asks. Enforcement is OpenCode's own
`permission` config in `opencode.json` and the per-agent permission blocks;
where it is bypassed, the Piper asks are the only gate. A broad request like
"finish this" is never a go-ahead. Go-aheads are recorded in the lane's
`build-log.md`; `project.md` holds standing policy notes only.
...
8. Before Ralph execution or source edits, verify the lane's checkout
   ...
   source edits there are routine. If writable access is absent, state what is
   required and wait.
