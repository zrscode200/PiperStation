...
assert_contains "$claude_hub/.claude/agents/implementer.md" "never"
...
if grep -R -n -E '[Pp]ermission [Pp]rofile|profile coverage|profile boundary|profile preference|`strict`|`local`|covers local|local profile' "$ROOT/core" "$ROOT/bootstrap/add-project.sh" "$ROOT/adapters" "$ROOT/generated" > "$TMP_ROOT/stale-profiles.log"; then cat "$TMP_ROOT/stale-profiles.log" >&2; fail "active instructions must use action classes, not permission profiles"; fi
