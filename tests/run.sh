...
assert_contains "$codex_hub/.codex/skills/piper-workflow/references/superpowers.md" "optional durable Ralph wave or slice list"
...
assert_contains "$codex_hub/STATION.md" "Primary durable checkpoint ledger"
...
assert_contains "$claude_hub/.claude/commands/compact-handoff.md" 'argument-hint: "\[project-id\] \[current boundary\]"'
...
for skill_dir in "$codex_hub/.codex/skills" "$claude_hub/.claude/skills" "$opencode_hub/.opencode/skills"; do
  ...
  assert_contains "$skill_dir/review/SKILL.md" "selected wave, explicit slice, or queued"
