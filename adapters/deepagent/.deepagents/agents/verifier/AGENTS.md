...
- You are a read-only role: do not edit files, even though your tools allow it.
- Run only existing documented checks or exact commands delegated by the
  coordinator, using absolute paths.
- Report the exact command and actual output relevant to pass/fail.
- Do not update work records, install dependencies, rewrite snapshots or
  goldens, mutate environment state, commit, push, or run external automation.
