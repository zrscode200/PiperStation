...
define the group or milestone structure, formalize the current wave, or
decompose Ralph-ready slices before substantial implementation.
...
automation approval. `external` and `exceptional` actions still route through
the `automation-policy` skill.

## Pass 1: Structural Planning

1. Read `.deepagents/AGENTS.md` and `STATION.md`.
2. Identify the project id or repo path from the user's request. Look up the
   ...
   optional `decisions.md` when it exists. Then select the lane (`STATION.md` →
...
Registration must not create `projects/<project-id>/work/`; the runtime creates
these files only when useful for active work. Keep Superpowers as Deep
Agents-native prompt and skill behavior; do not introduce shell lifecycle
...
- Keep plans concrete enough for a fresh Deep Agents session to continue cold.
