...
Ralph is prompt and skill behavior in Deep Agents. It selects the current
...
1. Read `.deepagents/AGENTS.md` and `STATION.md`. Look up the project in
...
3. Select one execution boundary: the wave, group review, explicit slice, or
   task matching the user's request if specified; otherwise the current
   boundary from `active-work.md` or `context-pack.md`; otherwise the top ready
   item in the optional queue. If the final wave in a group has landed and the
...
5. Verify the real project repo is writable in the active session. Registered
   repos live outside this hub: reach them by absolute path, keep the approval
   ...
   Confirm the active permission profile covers `local` project source edits;
   if not, route through `automation-policy` before editing.
...
one-off approval through `automation-policy`. Ralph prepares for compaction;
the runtime compacts on its own schedule.
...
When the gate runs, delegate to the `reviewer` subagent (defined under
`.deepagents/agents/`) through the `task` tool. The reviewer inspects the
actual changed code or diff and relevant surrounding code first, using the
active work record, build log, optional task queue, test output, and known
non-goals as supporting context. The reviewer reports correctness, regression,
security, reliability, missing-test, convention, and drift findings ordered by
severity with file and line references when possible.

The main Deep Agents session stays responsible for the work. Validate each
...
context, pause after updates and tell the user the state is compact-ready; a
fresh thread is the clean-context equivalent in this runtime.

## Post-Compact Resume

After compact or thread resume, start from designed anchors first:
...
cache, and `.git` directories. Remember the runtime snapshot: hub instructions
and skill listings in a resumed thread reflect thread start, not the current
files.
...
- Helper roles are instruction-constrained, not sandboxed: deny unexpected
  write or execute approvals arriving from a helper run.
