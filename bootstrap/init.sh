...
normalize_runtimes() {
  printf "%s\n" "$RUNTIME_INPUT" | tr ',' '\n' | while IFS= read -r runtime; do
    ...
    [ -n "$runtime" ] || continue
    case "$runtime" in codex|claude) printf '%s\n' "$runtime" ;; *) echo "Error: unsupported runtime: $runtime" >&2; exit 1 ;; esac
  done | sort -u
}
...
rel_runtime() { case "$1" in AGENTS.md|.codex/*|.piper/plugin/*|.piper/lib/*) printf codex ;; CLAUDE.md|.claude/*) printf claude ;; *) printf shared ;; esac; }
...
runtime_file_from_manifest_files() { files="$1"; { if grep -q '^AGENTS.md$\|^\.codex/\|^\.piper/plugin/\|^\.piper/lib/' "$files"; then printf 'codex\n'; fi; if grep -q '^CLAUDE.md$\|^\.claude/' "$files"; then printf 'claude\n'; fi; } | sort -u; }
